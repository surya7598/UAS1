...
if( empty( $_SESSION['id_user'] ) ){
	...
	if( isset( $_REQUEST['submit'] )){
		...
		$sql = mysqli_query($koneksi, "UPDATE transaksi SET jenis='$jenis', nama='$nama', alamat='$alamat', nohp='$nohp', email='$email', bayar='$bayar', kembali='$kembali', total='$total', tanggal=NOW(), id_user='$id_user' WHERE id_transaksi='$id_transaksi'");
...
<form method="post" action="" class="form-horizontal" role="form">
	...
	<div class="form-group">
		...
		<div class="col-sm-3">
			<input type="number" class="form-control" id="biaya" name="total" value="<?php echo $row['total']; ?>" placeholder="Total Bayar" required>
		</div>
	</div>
	<div class="form-group">
	...
	<div class="form-group">
		...
		<div class="col-sm-4">
			<input type="text" class="form-control" id="alamat" name="alamat" value="<?php echo $row['alamat']; ?>" placeholder="Alamat Pelanggan" required>
		</div>
	</div>
	<div class="form-group">
		<label for="nohp" class="col-sm-2 control-label">No HP</label>
		<div class="col-sm-4">
			<input type="text" class="form-control" id="nohp" name="nohp" value="<?php echo $row['nohp']; ?>" placeholder="No HP" required>
		</div>
	</div>
	<div class="form-group">
